...
DEBUG_PHOTOS = st.sidebar.checkbox("Debug player photos", False, key="dbg_photos_unique")
...
def _pick_ps_player_link(search_html: str, surname: str, team: str) -> str | None:
    ...
    for rel in links:
        # crude local context scoring by proximity in markup
        ...
        if rel.startswith("/player.php"): s += 1  # English site bias
        if s > best_score:
